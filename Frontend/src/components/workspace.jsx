...
const Workspace = () => {
    ...
    const analyzeCircuit = async () => {
        ...
        const results = state.components
            ...
            .map(component => ({
                ...
                voltage: (Math.random() * 12).toFixed(3),
                current: (Math.random() * 0.1).toFixed(4),
            }));
    ...
    const getTerminalDef = (component, terminalId) => {
        ...
        if (component.type === 'capacitor') defs = [{ id: 't1', x: -20, y: 0 }, { id: 't2', x: 20, y: 0 }];
        if (component.type === 'inductor') defs = [{ id: 't1', x: -20, y: 0 }, { id: 't2', x: 20, y: 0 }];
        if (component.type === 'ac-source') defs = [{ id: 'positive', x: 20, y: 0 }, { id: 'negative', x: -20, y: 0 }];
        if (component.type === 'transistor-npn') defs = [{ id: 'collector', x: 0, y: 20 }, { id: 'base', x: -20, y: 0 }, { id: 'emitter', x: 20, y: 0 }];
    ...
    const drawComponent = useCallback((component, ctx, selectedComponentId, selectedNodeId, wiringStartNodeId) => {
        if (!component || !component.position) {
            console.warn("Attempted to draw a malformed component:", component);
            return;
        }
        ctx.save();
